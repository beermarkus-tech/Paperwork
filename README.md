...
- While the filename field has focus (the on-screen keyboard is up), `#viewer-stage` — the PDF preview, along with the delete-page button and undo toast that live inside it — is hidden entirely (`#viewer.keyboard-open`), rather than being squeezed into a sliver above the keyboard. The toolbar, rename field, chips, and destination buttons shift up to use that space instead, and the preview comes back the moment the field loses focus.
...
- The whole chip row sits behind a "Labels" disclosure toggle, collapsed by default (it's new, and not everyone will use it) — the chevron rotates to show state, and whichever way you leave it is remembered (also in IndexedDB) for next time.
...
14. Confirm the "Labels" row starts collapsed the first time you open a document. Tap it to expand, confirm the chips and pencil button appear and the chevron rotates, then collapse it again. Fully reload the app and confirm it remembers whichever state you left it in.
15. Open a document and tap into the filename field to bring up the keyboard — confirm the PDF preview disappears entirely and the toolbar, filename field, chips, and destination buttons all shift up into the freed space. Tap away from the field (or tap the checkmark) to dismiss the keyboard and confirm the preview comes back.
