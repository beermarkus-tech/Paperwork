...
- No visual "saving" indicator on the rotate button — the rotation itself is already visible on the page the instant you tap, so a spinner for the background write just made the rotation itself look slow (it isn't). The debounce delay and the write still happen exactly as described above, just silently.
...
- The checkmark button itself is the progress/success indicator: it swaps to a spinner while the move is in flight, then turns green once it succeeds, and stays green until you switch to a different document (or page-flip back to this one later), at which point it resets to its normal white checkmark.
...
Phone behavior is untouched below this width — every rule here is gated behind a `min-width: 768px` media query (or, where CSS alone can't express the trigger, a JS check using the same breakpoint), so nothing in this section can change how the app looks or behaves on a phone.

- **Thumbnail grid**: 6 columns instead of 2, and the content column widens from 640px to 1600px.
- **Thumbnail captions**: page count alongside file size on both sizes ("4 pages · 1.0 MB"), once the page count is known.
- **Split/join selection**: a translucent navy overlay on selected thumbnails, with the join-order badge layered above it — on both sizes.
- **Cancel button**: a × appears in the header, left of the folder icon, whenever split or join is armed; hides on cancel or confirm.
- **Rename field focus**: instead of hiding the preview outright the way the phone's `keyboard-open` state does, focusing the rename field reveals the chip row and lets the (flex-sibling) preview shrink to make room — driven by `focus`/`blur` on the input rather than `visualViewport`, since a tablet's on-screen keyboard doesn't reliably resize the viewport the way a phone's does, and a physical keyboard never does. The phone's "hide the preview entirely" rule is explicitly scoped to `max-width: 767.98px` so it can never leak onto tablet even if `keyboard-open` does toggle there too.
- **Keyboard-adjacent browser chrome** that stays docked at the bottom of the screen even without the keyboard visible (an autofill suggestion bar, most commonly): `app.js` measures the actual gap live from `visualViewport` geometry and exposes it as a `--viewport-bottom-overlap` CSS custom property, which the tablet layout applies as extra bottom padding on the viewer — pushing the rename bar, chips, and destination row up above it by exactly however tall it actually is on that device, rather than guessing a fixed pixel value.

### Two-page landscape spread

Tablet **landscape** only — further gated on `(orientation: landscape) and (min-height: 700px)`, the screen's *short* axis rather than plain width, so a wide phone held sideways never qualifies. A multi-page document shows two pages side by side instead of one.

- `#viewer-pages` wraps one or two `.page-slot` elements (each holding its own canvas and its own floating rotate/delete toolbar); pan, zoom, and the page/document slide-transition all target this wrapper rather than a single canvas, so a spread's two pages always move together as one unit.
- `viewerState.pageNumber` is always the spread's *left* page (always odd — 1, 3, 5…); the right page, when shown, is always `pageNumber + 1`. Swiping steps by 2 instead of 1. A trailing odd-page-count document's last spread just shows one page (the right slot stays hidden) rather than padding with something blank.
- Rotate and delete are wired per-slot, so it's never ambiguous which page a tap affects — the left toolbar always acts on `pageNumber`, the right on `pageNumber + 1`.
- Rotation's debounced-save mechanism is keyed by `entry.name` + page number, not a single global slot like before spreads existed — a spread can have two independently-rotatable pages mid-save at once, so rotating the right page can no longer silently drop a still-pending save for the left one (or vice versa), and switching documents mid-save can't cross-contaminate two different documents' pending saves at the same page number either.
- Rotating the tablet between portrait and landscape while a document is open re-renders immediately; entering spread mode snaps to the odd page number that starts the pair containing whatever page was on screen, so the page you were looking at stays visible rather than the view jumping elsewhere.
...
8. Rotate a page a couple of times — the rotate button shows no spinner or other "saving" state, just the rotation itself applying instantly. Either wait ~2.5s or swipe away, then reopen that same document later (or fully reload the app) and confirm the rotation actually stuck, and that the thumbnail strip reflects the new orientation too.
...
### Tablet-specific (see "Tablet layout" above)

17. On a tablet, confirm the thumbnail grid shows 6 columns and the header stays sticky while scrolling. Arm split or join and confirm a × cancel button appears in the header, left of the folder icon, and that selected thumbnails get a translucent blue overlay with the join badge visible on top of it (not hidden behind it).
18. Open a document on tablet and tap into the rename field — confirm the chip row appears and the PDF preview shrinks to make room rather than disappearing entirely (unlike the phone behavior in step 14). Try this with both the on-screen keyboard and, if you have one, a connected physical keyboard.
19. Rotate the tablet to landscape with a multi-page document open (width comfortably over a phone's, e.g. an iPad or a 10"+ Android tablet) and confirm two pages show side by side, each with its own floating rotate/trash toolbar. Swipe to confirm it advances two pages at a time, rotate the left and right pages independently (rotating each applies instantly, with no spinner on either button) and confirm — after reopening the document, or a full reload — that each rotation actually saved to the correct page, and confirm pinch-zoom scales both pages together as one unit. Rotate the tablet back to portrait and confirm it drops back to one page, showing whichever page you were on.

The service worker registers with `updateViaCache: "none"` and calls `registration.update()` on load and whenever the app becomes visible again, so an update check hits the network for real rather than potentially being satisfied by GitHub Pages' own (uncontrollable — there's no way to set custom response headers there) caching of `sw.js` itself. Combined with `skipWaiting()`/`clients.claim()` in `sw.js`, a relaunch should reliably pick up a new deploy within a browser tab.

If you've already installed Paperwork to your home screen and an update still doesn't seem to take effect, the installed app (a WebAPK) has an extra layer of its own caching on top of the service worker's, and can get stuck on stale files regardless of the above. Uninstalling and reinstalling via "Add to Home screen" is the most reliable fix for that specific case — more reliable than in-place "Clear cache"/"Clear storage" from Android's App Info screen, which has been inconsistent in testing.
