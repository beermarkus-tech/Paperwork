...
14. Open a document. Confirm the chip labels are not visible while the PDF preview is showing, and that the destination buttons are visible. Tap into the filename field to bring up the keyboard — confirm the PDF preview disappears entirely, the chip labels appear, and the toolbar/filename field/destination buttons all shift up into the freed space. Dismiss the keyboard three different ways — tap the checkmark, tap a destination button, and use the phone's back gesture/button — and confirm the preview reliably comes back and the chips disappear again every time, including via the back gesture (that's the one that used to get stuck).
