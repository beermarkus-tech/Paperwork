...
- A plain, always-visible `<input type="date">` (`#rename-date-input`), defaulted to whatever date is already prefixed on the filename or today otherwise — tapping it opens the platform's own date picker directly, with no extra UI of ours layered on top. Picking a date fires the input's native `change` event, which writes it onto the filename in `YYYY-MM-DD` format, replacing an existing date prefix rather than stacking another one in front of it. Cancelling or dismissing the picker without choosing leaves the field, and therefore the filename, untouched — the platform picker's own Cancel/OK is the only such control in this flow.
